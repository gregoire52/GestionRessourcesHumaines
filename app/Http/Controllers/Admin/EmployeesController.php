...
class EmployeesController extends AdminBaseController
{
            ...
            ->filter(function($query) {
                if(request()->search['value']) {
                    $query->where('employeeID', 'LIKE', '%'.request()->search['value'].'%')
                            ...
                            ->orWhere('fatherName', 'LIKE', '%'.request()->search['value'].'%')
                            ->orWhere('fonction', 'LIKE', '%'.request()->search['value'].'%')
                            ->orWhere('gender', 'LIKE', '%'.request()->search['value'].'%')
							->orWhere('campus', 'LIKE', '%'.request()->search['value'].'%')
							->orWhere('site', 'LIKE', '%'.request()->search['value'].'%')
							->orWhere('situation', 'LIKE', '%'.request()->search['value'].'%')
							->orWhere('statut', 'LIKE', '%'.request()->search['value'].'%')
    ...
    public function store(CreateRequest $request)
    {
        ...
        try {
            ...
            $documents = ['cv', 'diplome', 'cni', 'cn', 'cj', 'iban', 'ipres', 'cm', 'lf', 'cbm', 'vcv'];
